...
AS_IF([test "x$with_netcdf" != xno],
  ...
  if test x"$acx_netcdf_ok" = xyes; then
    ...
    AC_MSG_RESULT()
    AC_CHECK_FUNC(nc_inq_grps,[acx_netcdf4_ok=yes;AC_MSG_RESULT([yes])],[AC_MSG_RESULT([no])])
  fi
...
DISTCHECK_CONFIGURE_FLAGS="$DISTCHECK_CONFIGURE_FLAGS --with-netcdf=$with_netcdf"
