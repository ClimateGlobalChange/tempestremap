...
public:
	...
	LatLonBox(
		...
		Type a_lon_width = static_cast<Type>(2.0 * M_PI)
	) :
		is_null(true),
	...
	///	<summary>
	///		Determine if this LatLonBox contains the given point, up to
	///		a prescribed machine epsilon.
	///	</summary>
	bool contains_eps(
		const Type & lat_pt,
		const Type & lon_pt,
		...
		// This box does not cross lon 360
		if ((lon_pt + eps >= lon[0]) && (lon_pt - eps <= lon[1])) {
			return true;
		}
		return false;
	}
