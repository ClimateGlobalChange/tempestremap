...
//############################################################################################################
    ...
    using default_interval_data_type = int;
//############################################################################################################
    template <typename numerical_type, typename interval_kind_ = closed, typename data_type_ = default_interval_data_type>
    struct interval
    {
    public:
        ...
        interval(value_type low, value_type high, data_type data)
            : low_{low}
            , high_{high}
            , data_{data}
        {
        ...
        {
            return lhs.low_ == other.low_ && lhs.high_ == other.high_ && lhs.data_ == other.data_;
        }
        ...
        /**
         *  Get the data associated with this interval.
         */
...
//############################################################################################################
    template <typename numerical_type = default_interval_value_type, typename data_type = default_interval_data_type, typename interval_type_ = interval <numerical_type, closed, data_type>>
    class node
    {
    private:
        using node_type = node <numerical_type, data_type, interval_type_>;
    ...
    public:
        ...
        friend lib_interval_tree::const_interval_tree_iterator <node <numerical_type, data_type, interval_type> >;
        friend lib_interval_tree::interval_tree_iterator <node <numerical_type, data_type, interval_type> >;
    ...
    public:
        ...
        /**
         *  Returns the data of the interval of this node
         */
        data_type data() const
        {
            return interval_.data();
        }
...
//############################################################################################################
    template <typename IntervalT = interval <int, closed, int>>
    class interval_tree
    {
    public:
        using interval_type = IntervalT;
        using value_type = typename interval_type::value_type;
        using data_type = typename interval_type::data_type;
        using node_type = node <value_type, data_type, interval_type>;
    ...
    public:
        ...
        {
            return find(ival, [](interval_type const& lhs, interval_type const& rhs){return lhs == rhs;});
        }
        /**
         *  Finds the first exact match.
         *
        ...
        {
            find_all(ival, on_find, [](interval_type const& lhs, interval_type const& rhs){return lhs == rhs;});
        }
        template <typename FunctionT>
        ...
        {
            return find_next_in_subtree(from, ival, [](interval_type const& lhs, interval_type const& rhs){return lhs == rhs;});
        }
